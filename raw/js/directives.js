	    ...
	    return {
	      ...
	      link: function postLink(scope, element, attrs) {
	        ...
	        scope.scales = [
	        	...
	        	{
	        		type : 'Linear (numeric)',
	        		value : d3.scale.linear().range(["#ff0000", "#08306b"]),
	        		reset : function(){ this.value.range(["#ff0000", "#08306b"]); },
	        		update : linearUpdate
	        	}
	        ];
	        ...
	        scope.colorScale = scope.scales[2];
...
.directive('snshare', function () {
    return {
      restrict: 'E',
      replace:true,
      template :  '<div class="row">' +
                    '<form class="form-search col-lg-12">' +
                      '<button class="btn btn-success form-control" ng-class="{disabled:!mode.label}" ng-click="share()">share it</button>' +
                    '</form>' +
                  '</div>',

      link: function postShare(scope, element, attrs) {
